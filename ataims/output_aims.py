...
class OutputAims(Output):
    ...
    async def prepare_workflow_outputs(
        self, file_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        response = await util.fetch(
            "/upload-workflow", method="POST", body=file_data["content"]
        )
        if not response.ok:
    ...
    def _parse(
        self, files_data: List[Dict[str, Any]], is_workflow: bool = False
    ) -> "OutputAims":
        import os
        ...
        for file_data in files_data:
            ...
            is_band_file = (
                "band" in file_name if is_workflow else file_name.startswith("band")
            )
            is_gw_band_file = (
                "GW_band" in file_name
                if is_workflow
                else file_name.startswith("GW_band")
            )
        ...
        for file_data in files_data:
            ...
            if (
                re.match(r"KS_DOS_total(_tetrahedron)?.dat", file_name)
                or re.match(r"_proj_dos(_tetrahedron)?\.dat(\.no_soc)?", file_name)
                or re.match(
                    r"atom_proj(ected)?_dos(_tetrahedron)?_[A-Z][a-z]?[0-9]{4}\.dat(\.no_soc)?",
                    file_name,
                )
            ):
                ...
                r"_proj_dos(_tetrahedron)?_spin_(up|dn)\.dat(\.no_soc)?", file_name
            ):
                alt_file_name = file_name.replace("_spin_up", "").replace(
                    "_spin_dn", ""
                )
                ...
                r"absorption(_soc)?(_Lorentzian)?(_Tetrahedron)?_[0-9].[0-9]{4}(_x_x)?(_y_y)?(_z_z)?.out",
                file_name,
            ):
                self.files["absorption"][file_name] = self.get_tabulated_data_fhi_aims(
                    lines
                )
                self.filled_up = True
            elif re.match(
                r"dielectric_function(_soc)?(_Lorentzian)?(_Tetrahedron)?_[0-9].[0-9]{4}(_x_x)?(_y_y)?(_z_z)?(_x_y)?(_x_z)?(_y_z)?.out",
                file_name,
            ):
                self.files["dielectric"][file_name] = self.get_tabulated_data_fhi_aims(
                    lines
                )
                self.filled_up = True
            elif (is_band_file or is_gw_band_file) and file_name.endswith(".out"):
                self.add_bs_file_data(
                    file_name, self.segments_map, lines, is_gw_band_file, workflow_idx
                )
                self.filled_up = True
    ...
    def add_bs_file_data(
        ...
        file_name_token = (
            file_name[: segment_num_ind[0]] + "***" + file_name[segment_num_ind[1] :]
        )
        ...
        for line in lines:
            if len(line.strip()) > 1:
                ...
                segment_data["band_energies"][spin_index].append(
                    [e for i, e in enumerate(a[4:]) if i % 2 == 1]
                )

    def get_bs_info(self) -> Dict[str, Any]:
        if not hasattr(self, "bs_info"):
            if self.control_in:
                self.bs_info = self.find_lacking_segments(
                    self.control_in, self.segments_map
                )
            else:
    ...
    @staticmethod
    def find_lacking_segments(
        control_in: Optional[List[str]], segments_map: Dict[str, Any]
    ) -> Dict[str, Any]:
        lacking_segments = []
        bs_data = []

        if control_in:
            lacking_segments = OutputAims.add_segment_labels_to_bs(
                control_in, segments_map
            )
    ...
    @staticmethod
    def add_segment_labels_to_bs(
        lines: List[str], segments_map: Dict[str, Any]
    ) -> List[str]:
        segments_labels = []
        lacking_segments = []
        ...
        for loop in self.scf_loops:
            ...
            if has_iter:
                data_series_iteration = {
                    "eigenvalues": {
                        "label": "Change of Eigenvalues",
                        "color": "rgb(0,157,220)",
                        "data": [],
                    },
                    "totalEnergy": {
                        "label": "Change of Total Energy",
                        "color": "rgb(242,100,48)",
                        "data": [],
                    },
                }
                if self.run_time_choices["spin"] == "collinear":
                    data_series_iteration["chargeDensityUp"] = {
                        "label": "Change of Charge Density",
                        "color": "rgb(42,45,52)",
                        "data": [],
                    }
                    data_series_iteration["chargeDensityDown"] = {
                        "label": "Change of Spin Density",
                        "color": "rgb(84,45,52)",
                        "data": [],
                    }
                else:
                    data_series_iteration["chargeDensity"] = {
                        "label": "Change of Charge Density",
                        "color": "rgb(42,45,52)",
                        "data": [],
                    }
    ...
    def get_relaxation_series(self) -> None:
        ...
        for i, loop in enumerate(self.scf_loops, 1):
            ...
            if loop.get("isConverged", False):
                iters.append(i)
                if (
                    "finalScfEnergies" in loop
                    and "totalEnergy" in loop["finalScfEnergies"]
                ):
                    ...
                    logging.warning(
                        f"Missing 'finalScfEnergies' or 'totalEnergy' in loop {i}"
                    )
        ...
        if not iters:
            logging.warning(
                "No converged iterations found. Check if this is a relaxation calculation."
            )
            return
    ...
    def get_results_quantities(self) -> Dict[str, Any]:
        ...
        if last_converged >= 0:
            ...
            quantities["Total Energy (eV)"] = last_loop["finalScfEnergies"][
                "totalEnergy"
            ]
            ...
            if (
                ...
                quantities["Cell Volume (&#197;<sup>3</sup>)"] = float(
                    get_parallelepiped_volume(self.structure_in.lat_vectors)
                )
    ...
    def get_input_files_map(self) -> Dict[str, str]:
        ...
        input_files_map = {}
        if self.inputs["geometryIn"]:
            input_files_map["geometry.in"] = line_array_to_text(
                self.inputs["geometryIn"]
            )
    ...
    def get_ks_ev(self, file_text: str) -> List[List[float]]:
        ...
        for line in lines:
            ...
            if (
                "State" in line
                and "Occupation" in line
                and "Eigenvalue" in line
                and not spin_collinear
            ):
                ks_ev = []
                found_ks_ev = True
    ...
    def parse_until(
        self, reg_exs: List[re.Pattern], stop_re: re.Pattern
    ) -> Optional[List[str]]:
        lines = []
    ...
    def get_lines_in_between(
        self, start_re: re.Pattern, stop_re: re.Pattern
    ) -> Optional[List[str]]:
        lines = []
    ...
    def get_electron_info(self) -> Optional[Dict[str, Dict[str, Any]]]:
        ...
        line = self.wait_for(
            re.compile(r"Highest occupied state|Reaching maximum number")
        )
        if line is None:
            return None
    ...
    def get_iteration(self) -> Tuple[Optional[bool], Optional[Dict[str, Any]]]:
        ...
        is_converged = False
        line = self.wait_for(
            r"(Begin self-consistency iteration|SELF-CONSISTENCY CYCLE DID NOT CONVERGE)"
        )
        if line is None:
            return None, None
        ...
        if line:
            ...
            is_converged = self.check_for_until(
                re.compile(r"Self-consistency cycle converged"),
                re.compile(r"End self-consistency iteration"),
            )
    ...
    def get_scf_cycle_md_light(self) -> Dict[str, Any]:
        ...
        scf_energies = self.scf_cycle["iterations"][iter]["scfEnergies"] = scf_energies[
            "scfEnergies"
        ]
        electron_info = self.get_electron_info()
        ...
        is_converged = self.check_for_until(
            re.compile(r"Self-consistency cycle converged"),
            re.compile(
                r"End self-consistency iteration|Reaching maximum number of scf iterations"
            ),
        )
    ...
    def get_formula_unit(structure: Structure) -> str:
        ...
        formula_unit = "".join(
            f"{key}{value if value > 1 else ''}" for key, value in species.items()
        )
        return formula_unit
    ...
    def parse_updated_geo(self) -> Optional[Dict[str, Any]]:
        line = self.wait_for(
            re.compile(
                r"Geometry optimization: Attempting to predict improved coordinates\."
            )
        )
        if line is None:
            return None
        ...
        if "not yet converged." in line:
            ...
            if line is None:
                return None
            geo_lines = self.parse_until(
                [re.compile(r"atom\s+"), re.compile(r"lattice_vector\s+")],
                re.compile(r"---------"),
            )
            return {
            ...
            if line is None:
                return None
            return {"force": max_force_component, "geo": self.current_geometry}

    def get_mulliken_pop(self, soc: bool = False) -> Optional[Dict[str, List[float]]]:
        mulliken_charge = {}
        if soc:
            line = self.wait_for(
                re.compile(r"Performing scalar-relativistic Mulliken charge analysis")
            )
            if line is None:
                return None
            mulliken_charge["sr"] = self.read_mulliken()
            line = self.wait_for(
                re.compile(r"Performing spin-orbit-coupled Mulliken charge analysis")
            )
            if line is None:
                return None
            ...
            line = self.wait_for(
                re.compile(r"Performing Mulliken charge analysis on all atoms")
            )
            if line is None:
                return None
    ...
    def get_final_scf_energies_forces(self) -> Dict[str, Any]:
        ...
        line = next(self.line_it)
        final_energies["totalEnergyCorrected"] = float(
            re.findall(self.float_regex, line)[0]
        )
        line = next(self.line_it)
        final_energies["electronicFreeEnergy"] = float(
            re.findall(self.float_regex, line)[0]
        )
        ...
        if self.run_time_choices.get("has_forces", False):
            ...
            for _ in range(self.system_information["nAtoms"]):
                ...
                forces.append(force)
            max_force_component = (
                max(max(abs(f) for f in force) for force in forces) if forces else None
            )
    ...
    def check_for_until(
        self, check_re: re.Pattern, stop_re: re.Pattern
    ) -> Optional[bool]:
        for line in self.line_it:
        ...
        while True:
            ...
            if is_starting:
                ...
                if output_level == "normal":
                    ...
                    logger.warning(
                        f"Warning: Unknown output_level '{output_level}'. Using normal parsing."
                    )
                    scf_cycle = self.get_scf_cycle(scf_cycle)
                ...
                scf_cycle["finalScfEnergies"] = final_scf_energies_forces.get(
                    "finalScfEnergies", {}
                )
                scf_cycle["forces"] = final_scf_energies_forces.get("forces", [])
                scf_cycle["maxForceComponent"] = final_scf_energies_forces.get(
                    "maxForceComponent"
                )
    ...
    def get_calculation_info(self) -> Dict[str, Dict[str, Any]]:
        ...
        calculation_info_reg_exs = [
            (
                "codeVersion",
                "Code Version",
                re.compile(r"FHI-aims version\s+:\s+([0-9]+)"),
            ),
            (
                "commitNumber",
                "Commit Number",
                re.compile(r"Commit number\s+:\s+([0-9,a-z]*)"),
            ),
            (
                "numberOfTasks",
                "Number of Tasks",
                re.compile(r"Using\s+([0-9]*)\s+parallel tasks\."),
            ),
        ]
        for line in self.line_it:
    ...
    def get_memory(self) -> Optional[Dict[str, Dict[str, Any]]]:
        ...
        if line is None:
            return None
        m = re.findall(self.float_regex, line)
        memory["peakMemory"] = {"value": m[0], "info": "Peak memory among tasks (MB)"}
